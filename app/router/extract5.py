...
class ExtractURL():
    ...
    def extract_info(url : str):
        ...
        key = "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"
        openai.api_key = key
        openai.Model.list()
        ...
        response = completion.choices[0].message
        row =row +response["content"].split("\n")
        row.append("html:"+soup.prettify())
        row.append("text:"+text)
        row.append("URL:"+url)
        fields = ["title","organization","deadline","type","benefits/value","educationLevel","majors","link","requirements","address","html_file","raw_text","url"]
        count=0
        scholar_dict ={}
        for each in row:
            if len(each)<=1:
